...
Actual GitHub upload/publication and the legacy guest run still require the runner's QEMU, OpenCore configuration, macOS Installer, SSH credentials, and release credentials. HTTPS registry Bottle URLs are downloaded to a temporary path and SHA256 checked before Homebrew receives them; Homebrew's download cache is never impersonated.
